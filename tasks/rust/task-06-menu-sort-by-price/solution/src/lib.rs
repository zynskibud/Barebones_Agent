...
/// Return a new list ordered from the lowest price to the highest.
/// Items with the same price keep their menu order.
pub fn sort_by_price(menu: &[MenuItem]) -> Vec<&MenuItem> {
    let mut items: Vec<&MenuItem> = menu.iter().collect();
